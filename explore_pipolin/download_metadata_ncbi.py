...
from explore_pipolin.utilities.external_tools import subprocess_with_retries, EmptyResult
...
class MultispeciesResult(Exception):
    pass


def check_multispecies(esearch_result):
    fetched = subprocess_with_retries(['efetch', '-format', 'docsum'], text=True,
                                      input=esearch_result.stdout, stdout=subprocess.PIPE)
    extracted = subprocess_with_retries(['xtract', '-pattern', 'DocumentSummary', '-element', 'Title'],
                                        text=True, input=fetched.stdout, stdout=subprocess.PIPE)
    if 'MULTISPECIES' in extracted.stdout:
        raise MultispeciesResult
...
def get_assembly_acc_and_species_name(assembly_info):
    ...
    info_strings = assembly_info_extracted.stdout.strip().split(sep='\n')
    # There might be old release assemblies as well, but we need the latest!
    # TODO: sometimes there are several different assemblies. For releases check the last number only!!!
...
def get_and_filter_seqs_info(assembly_info):
    seqs_info = subprocess_with_retries(['elink', '-target', 'nucleotide', '-name', 'assembly_nuccore_insdc'],
                                        text=True, input=assembly_info, stdout=subprocess.PIPE)
    seqs_info_fetched = subprocess_with_retries(['efetch', '-format', 'docsum'], text=True,
    ...
    seqs_info_filtered = subprocess_with_retries(
        ...
        text=True, input=seqs_info_fetched.stdout, stdout=subprocess.PIPE)
    plasmids_excluded = subprocess.run(['grep', '-v', 'plasmid'], text=True,
                                       input=seqs_info_filtered.stdout, stdout=subprocess.PIPE)
    return plasmids_excluded.stdout
...
def download_metadata_ncbi(in_file, out_file, id_type):
    """
    ...
    with open(out_file, 'w') as ouf:
        ...
        for num, gb_id in enumerate(gb_ids):
            ...
            try:
                assembly_info = get_assembly_info(gb_id, id_type)
                ...
                print(f'NO RESULTS for the query {gb_id} !!!')
                continue
            except MultispeciesResult:
                print(f'MULTISPECIES result for the query {gb_id}. Pass !!!')
