...
def run_aragorn(genome, aragorn_results):
    ...
    with open(os.path.join(aragorn_results, define_genome_id(genome_path=genome) + '.batch'), 'w') as ouf:
        subprocess.run(['aragorn', '-l', '-w', genome], stdout=ouf)


def blast_genome_against_seq(genome, seq, seq_type, output_dir):
    os.makedirs(output_dir, exist_ok=True)
    with open(os.path.join(output_dir, define_genome_id(genome_path=genome) + '.fmt5'), 'w') as ouf:
        if seq_type == 'protein':
