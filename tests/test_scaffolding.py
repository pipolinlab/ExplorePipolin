...
def _create_genome_with_contigs(contigs_schemes: Sequence[str]) -> Genome:
    ...
    for i in range(len(contigs_schemes)):
        contigs.append(Contig(ContigID(f'CONTIG_{i}'), len(contigs_schemes[i]) // 3 * 10))
...
def _add_features_to_genome(contigs_schemes, genome):
    for i, sch in enumerate(contigs_schemes):
        for triplet_start in range(0, len(sch), 3):
            ...
            feature = Feature(Range(feature_start, feature_start + 10), Strand.FORWARD, ContigID(f'CONTIG_{i}'), genome)
            if triplet == _ATT:
                feature = AttFeature(Range(feature_start, feature_start + 10), Strand.FORWARD,
                                     ContigID(f'CONTIG_{i}'), genome, att_id=0)
...
class TestScaffolder(unittest.TestCase):
    ...
    def test_genome1(self):
        ...
        exp = [Pipolin(PipolinFragment(Range(0, 30), ContigID('CONTIG_0'), genome))]

        self._check_pipolins(exp, scaffold(genome))

    def test_genome2(self):
        genome = create_genome_from_scheme('---att---pol---')
        exp = [Pipolin(PipolinFragment(Range(0, 50), ContigID('CONTIG_0'), genome))]

        self._check_pipolins(exp, scaffold(genome))

    def test_genome3(self):
        genome = create_genome_from_scheme('---att---pol---att---')
        exp = [Pipolin(PipolinFragment(Range(0, 70), ContigID('CONTIG_0'), genome))]

        self._check_pipolins(exp, scaffold(genome))

    def test_genome4(self):
        genome = create_genome_from_scheme('---att---pol---...---att---')
        f1 = PipolinFragment(Range(0, 50), ContigID('CONTIG_0'), genome)
        f2 = PipolinFragment(Range(0, 30), ContigID('CONTIG_1'), genome)
        exp = [Pipolin(f1, f2)]
    ...
    def test_genome5(self):
        genome = create_genome_from_scheme('---att---...---pol---...---att---')
        f1 = PipolinFragment(Range(0, 30), ContigID('CONTIG_0'), genome)
        f2 = PipolinFragment(Range(0, 30), ContigID('CONTIG_1'), genome)
        f3 = PipolinFragment(Range(0, 30), ContigID('CONTIG_2'), genome)
    ...
    def test_genome7(self):
        genome = create_genome_from_scheme('---att---...---pol---att---att(t)---')
        f1 = PipolinFragment(Range(0, 30), ContigID('CONTIG_0'), genome)
        f2 = PipolinFragment(Range(0, 70), ContigID('CONTIG_1'), genome)
        exp = [Pipolin(f1, f2)]
    ...
    def test_genome8(self):
        genome = create_genome_from_scheme('---att---pol---...---att---att(t)---')
        f1 = PipolinFragment(Range(0, 50), ContigID('CONTIG_0'), genome)
        f2 = PipolinFragment(Range(0, 50), ContigID('CONTIG_1'), genome)
        exp = [Pipolin(f1, f2)]
    ...
    def test_genome9(self):
        genome = create_genome_from_scheme('---att---pol---att---...---att(t)---')
        f1 = PipolinFragment(Range(0, 70), ContigID('CONTIG_0'), genome)
        f2 = PipolinFragment(Range(0, 30), ContigID('CONTIG_1'), genome)
        exp = [Pipolin(f1, f2)]
    ...
    def test_genome10(self):
        ...
        f1 = PipolinFragment(Range(0, 30), ContigID('CONTIG_0'), genome)
        f2 = PipolinFragment(Range(0, 30), ContigID('CONTIG_1'), genome)
        f3 = PipolinFragment(Range(0, 50), ContigID('CONTIG_2'), genome)
        exp = [Pipolin(f1, f2, f3)]
    ...
    def test_genome18(self):   # two pipolins
        ...
        p1 = Pipolin(PipolinFragment(Range(0, 70), ContigID('CONTIG_0'), genome))
        p2 = Pipolin(PipolinFragment(Range(60, 130), ContigID('CONTIG_0'), genome))
    ...
    def test_genome23(self):
        ...
        exp = [Pipolin(PipolinFragment(Range(0, 110), ContigID('CONTIG_0'), genome))]
